...
func find_path(start, target):
	...
	var index_start = get_point_index(Vector3(grid_start))
	var index_end = get_point_index(Vector3(grid_end))
	...
	for point in astar_path:
		world_path.append(grid_map.map_to_local(Vector3i(point.x, point.y, point.z)))	
